...
void SensorsListLayout::addButton(Sensor* s) {
    ...
    if(!sensorsLabel->isHidden()) {
        sensorsLabel->hide();
    }

    CustomButton* button = new CustomButton(s->getName(), s); // PROBABILI PROBLEMI DI MEMORIA
...
void SensorsListLayout::clearLayout(QLayout*) {
    QLayoutItem* item;
...
QLayout* SensorsListLayout::getLayout() const {
